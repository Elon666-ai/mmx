 ...
 * @property {number} [maxBitrate] - Max bitrate in kbps (adds b=AS to SDP).
 * @property {boolean} [audioOnly] - If true, do not request video.
...
class MediaMTXWebRTCReader {
  ...
  /**
   * Helper to insert b=AS:<bitrate> into the SDP to limit bandwidth (trigger Simulcast switch).
   * @param {string} sdp - The original SDP.
   * @param {number} bitrateKbps - The max bitrate in kbps.
   */
  static #limitBandwidth(sdp, bitrateKbps) {
    ...
    if(videoSectionIndex !== -1) {
        // Insert b=AS:<bitrate> after m=video line
        // b=AS is Application Specific bandwidth in kbps
        lines.splice(videoSectionIndex + 1, 0, `b=AS:${bitrateKbps}`);
    }
  ...
  #handleError(err) {
    if (this.state === 'running' || this.state === 'getting_codecs') {
  ...
  #setupPeerConnection(iceServers) {
    ...
    // Modified Transceiver Logic for Audio-Only and Standard modes
    if (this.conf.audioOnly) {
        this.pc.addTransceiver('audio', { direction: 'recvonly' });
        // Intentionally do not add video transceiver
    } else {
    ...
    return this.pc.createOffer()
      .then((offer) => {
        ...
        // 1. Apply codec edits (Opus/L16 etc)
        sdp = MediaMTXWebRTCReader.#editOffer(sdp, this.nonAdvertisedCodecs);
        
        // 2. Apply Bandwidth Limit for Simulcast (b=AS)
        if (this.conf.maxBitrate && this.conf.maxBitrate > 0) {
